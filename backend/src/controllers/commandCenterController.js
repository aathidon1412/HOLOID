...
const listTransferMetrics = async (req, res) => {
  const activeFilter = {
    status: { $in: ["requested", "dispatched", "in_transit"] },
    assignedAmbulance: { $ne: null }
  };

  const [activeTransfers, inTransit, awaitingDriver, accepted] = await Promise.all([
    Transfer.countDocuments(activeFilter),
    Transfer.countDocuments({ ...activeFilter, status: "in_transit" }),
    Transfer.countDocuments({ ...activeFilter, dispatchStatus: "pending_driver" }),
    Transfer.countDocuments({ ...activeFilter, dispatchStatus: "accepted" })
  ]);
