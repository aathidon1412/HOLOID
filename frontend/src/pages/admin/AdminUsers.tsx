...
const AdminUsers = () => {
  ...
  const { data: users = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Backend controller listAllUsers for HOSPITAL_ADMIN already scopes to their hospital operational users
      ...
      <div className="p-6 space-y-6">
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <div className="flex gap-4 items-center flex-wrap">
                ...
                <p className="text-sm text-muted-foreground font-medium">Managing Users for {currentUser?.hospital ? 'your hospital' : 'HOLOID'}</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="px-5 py-4 text-left text-xs font-bold text-muted-foreground uppercase tracking-wider">Role</th>
              ...
              {!isLoading && filteredUsers.map((u) => {
                 ...
                 return (
                  <tr key={u._id} className="hover:bg-accent/5 transition-colors">
                    ...
                    <td className="px-5 py-4 text-muted-foreground font-medium">{formatRoleLabel(u.role)}</td>
        ...
        <div className="flex items-center justify-between px-2">
             <p className="text-xs text-muted-foreground">Users are automatically filtered to your hospital only.</p>
             <p className="text-xs font-medium text-muted-foreground">{filteredUsers.length} user(s) found</p>
        </div>
      </div>
    </div>
  );
